...
class ObjectDetectionNode : public rclcpp::Node {
    ...
    private:
        ...
        // Text encoder output for the queries, computed once at startup: {max_queries, query_embed_dim_}
        std::vector<float> cached_query_embeds_;
        int64_t query_embed_dim_ = 0;
        const int max_queries = 16;
        const int max_tokens = 16;
        std::vector<std::string> text_queries_ = {
            "table", "chair", "television", "cat", "window", "door", "person", "phone", "dog", "cat", "laptop", "keyboard", "mouse", "book", "cup", "bottle"
        };
        ...
        void load_tokenizer(const std::string& model_name) {
            ...
            RCLCPP_INFO(this->get_logger(), "Tokenizer loaded successfully from: %s (%.3f seconds)", tokenizer_path.c_str(), (this->now() - start_time).seconds());
        }

        void load_model(const std::string& model_name) {
            rclcpp::Time start_time = this->now();
            std::string model_path = "/home/bmacraze/ros2_kilted_ws/src/object_detection/models/" + model_name + "/image_detector_int8.onnx";
            ...
            RCLCPP_INFO(this->get_logger(), "ONNX model loaded successfully from: %s (%.3f seconds)", model_path.c_str(), (this->now() - start_time).seconds());
        }
        ...
        void process_text_prompts(std::vector<int64_t>& input_ids, std::vector<int64_t>& attention_mask) {
            ...
            RCLCPP_INFO(this->get_logger(), "Text prompts processed and cached for ONNX inference. (%.3f seconds)", (this->now() - start_time).seconds());
        }

        void encode_text_prompts(const std::string& model_name) {
            rclcpp::Time start_time = this->now();
            std::string model_path = "/home/bmacraze/ros2_kilted_ws/src/object_detection/models/" + model_name + "/text_encoder.onnx";
            RCLCPP_INFO(this->get_logger(), "Encoding text prompts with: %s", model_path.c_str());

            // The queries never change, so the text encoder only runs here; its session is freed on return
            Ort::SessionOptions session_options;
            session_options.SetIntraOpNumThreads(4);
            Ort::Session text_session(env_, model_path.c_str(), session_options);
            ...
            input_tensors.push_back(Ort::Value::CreateTensor<int64_t>(
                memory_info, cached_input_ids_.data(), cached_input_ids_.size(), text_shape.data(), text_shape.size()
            ));
            input_tensors.push_back(Ort::Value::CreateTensor<int64_t>(
                memory_info, cached_attention_mask_.data(), cached_attention_mask_.size(), text_shape.data(), text_shape.size()
            ));

            const char* input_names[] = {"input_ids", "attention_mask"};
            const char* output_names[] = {"query_embeds"};
            auto output_tensors = text_session.Run(
                Ort::RunOptions{nullptr}, input_names, input_tensors.data(), input_tensors.size(),
                output_names, 1
            );

            auto embeds_info = output_tensors[0].GetTensorTypeAndShapeInfo();
            query_embed_dim_ = embeds_info.GetShape()[1];
            const float* embeds = output_tensors[0].GetTensorData<float>();
            cached_query_embeds_.assign(embeds, embeds + embeds_info.GetElementCount());
            RCLCPP_INFO(this->get_logger(), "Text prompts encoded and cached. (%.3f seconds)", (this->now() - start_time).seconds());
        }
        ...
        void image_callback(const sensor_msgs::msg::Image::SharedPtr msg) {
            ...
            // Precomputed text embeddings: {batch, num_queries, embed_dim}
            std::vector<int64_t> query_shape = {1, static_cast<int64_t>(max_queries), query_embed_dim_};
            Ort::Value query_tensor = Ort::Value::CreateTensor<float>(
                memory_info_, cached_query_embeds_.data(), cached_query_embeds_.size(), query_shape.data(), query_shape.size()
            );

            std::vector<Ort::Value> input_tensors;
            input_tensors.push_back(std::move(img_tensor));
            input_tensors.push_back(std::move(query_tensor));

            const char* input_names[] = {"pixel_values", "query_embeds"};
            ...
            // Stamp with the source frame's time so the visualizer can pair detections with the right image
            ...
            // Publish even when empty so the visualizer's sync doesn't stall on frames with no detections
            detections_pub_->publish(detections_msg);
            RCLCPP_DEBUG(this->get_logger(), "Processed image and published %zu detections. (%.3f seconds)", 
                        detections_msg.detections.size(), (this->now() - start_time).seconds());
        }
};
